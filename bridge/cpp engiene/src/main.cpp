...
void generate_mock_template(const std::string& roll, uint8_t* template_out) {
    ...
    // Simple reproducible mock template: fill with ascii values of the roll number repeating
...
void do_enroll() {
    ...
    std::string roll;
    std::cin >> roll;
    std::strncpy(student.roll_number, roll.c_str(), sizeof(student.roll_number) - 1);
    ...
    std::cout << BOLD << YELLOW << "\n  >>> [ACTION REQUIRED] Please touch the MacBook Touch ID scanner to enroll... <<<\n" << RESET;
    std::string prompt_str = "Enroll fingerprint for student " + name;
    bool touch_ok = macos_touch_id_authenticate(prompt_str.c_str());
    
    if (!touch_ok) {
        print_error("MacBook Touch ID enrollment failed or was cancelled. Enrollment aborted.");
        return;
    }
...
void do_scan() {
    ...
    if (students.empty()) {
        print_warning("No students currently enrolled in master database. Please enroll students first!");
        return;
    }
    
    std::cout << "  Available students to simulate:\n";
    for (size_t i = 0; i < students.size(); ++i) {
        std::cout << "  [" << i + 1 << "] " << students[i].name << " (" << students[i].roll_number 
    ...
    std::cout << BOLD << YELLOW << "\n  >>> [ACTION REQUIRED] Please touch the MacBook Touch ID scanner... <<<\n" << RESET;
    
    std::string prompt_str = "Authorize gate scan for student " + std::string(selected_student.name);
    bool touch_ok = macos_touch_id_authenticate(prompt_str.c_str());
    ...
    if (home_exists(match.roll_number)) {
        std::cout << BOLD << GREEN << "  [RETURNING FROM HOME] Student detected on approved home leave.\n" << RESET;
    ...
    if (!has_scanned_today) {
        ...
        std::strncpy(log_entry.timestamps[log_entry.timestamp_count], now_time.c_str(), sizeof(log_entry.timestamps[log_entry.timestamp_count]) - 1);
    ...
    if (is_after_curfew() && is_in && match.is_hosteller) {
        ...
        std::cout << BOLD << RED << "  [CURFEW WARNING] Late return detected! (Curfew is 18:30)\n" << RESET;
    }
    ...
    if (write_ok) {
        std::cout << BOLD << GREEN << "  [LOGGED] " << match.name << " is marked " << status_str 
                  << " (Gate count: " << log_entry.gate_count << ", Purpose: " << purpose << ")\n" << RESET;
    } else {
...
void do_view_logs() {
    ...
    for (const auto& entry : entries) {
        std::string last_time = (entry.timestamp_count > 0) ? entry.timestamps[entry.timestamp_count - 1] : "N/A";
        std::cout << std::left 
                  << std::setw(12) << entry.roll_number 
                  << std::setw(20) << entry.name 
                  << std::setw(8) << entry.year 
                  << std::setw(15) << entry.reason 
                  << std::setw(8) << entry.gate_count 
                  << std::setw(8) << entry.status 
...
void do_curfew_check() {
    ...
    for (const auto& log : entries) {
        ...
        StudentRecord student;
        if (!student_get(log.roll_number, student)) continue;
        ...
        if (is_anomaly) {
            ...
            std::cout << std::left 
                      ...
                      << std::setw(12) << role_str 
                      << std::setw(10) << log.status 
                      << student.phone_number << "\n";
        }
    }
    
    if (!anomaly_found) {
        print_success("All scanned students are fully accounted for and compliant!");
    }
}
...
void do_home_list() {
    ...
    for (const auto& r : list) {
        std::cout << std::left 
                  << std::setw(12) << r.roll_number 
                  << std::setw(20) << r.name 
                  << std::setw(8) << r.year 
                  << std::setw(15) << r.date_of_leaving 
                  << std::setw(15) << r.time_of_leaving 
                  << r.phone_number << "\n";
    }
}
...
void do_batch_promote() {
    ...
    if (choice == 1) {
        ...
        std::cout << BOLD << GREEN << "\n  Successfully promoted " << promoted << " students in batch " << batch << "!" << RESET << "\n";
    } else if (choice == 2) {
        int promoted = batch_promote_all();
        std::cout << BOLD << GREEN << "\n  Successfully promoted " << promoted << " students globally!" << RESET << "\n";
    } else {
        print_error("Invalid selection.");
    }
}
...
void do_view_master_db() {
    ...
    for (const auto& student : students) {
        ...
        std::cout << std::left 
                  << std::setw(12) << student.roll_number 
                  << std::setw(20) << student.name 
                  << std::setw(10) << student.program 
                  << std::setw(8) << student.batch 
                  << std::setw(6) << student.year 
                  << std::setw(12) << role_str 
                  << std::setw(15) << student.phone_number 
                  << "0x" << std::hex << std::setw(8) << std::setfill('0') << hash 
                  << std::dec << std::setfill(' ') << "\n";
    }
}
...
int main() {
    ...
    if (!engine_init(db_root.c_str())) {
        std::cerr << "CRITICAL ERROR: Failed to initialize C++ Database Engine!" << std::endl;
        return 1;
    }
    
    while (true) {
        std::cout << BOLD << MAGENTA << "\n======================================================================\n";
        std::cout << "             CAMPUS BIOMETRIC GATE SYSTEM - CONSOLE CONTROLLER\n";
        std::cout << "======================================================================" << RESET << "\n";
        std::cout << "  Current Date: " << get_current_date_string() << " | Current Time: " << get_current_time_string() << "\n\n";
