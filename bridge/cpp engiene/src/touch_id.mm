...
bool macos_touch_id_authenticate(const char* prompt_reason) {
    ...
    // We must run this on a context that has a run loop or wait synchronously
    @autoreleasepool {
        LAContext *context = [[LAContext alloc] init];
        NSError *error = nil;
        
        // Provide a default prompt if none given
        NSString *reason = prompt_reason ? [NSString stringWithUTF8String:prompt_reason] : @"Authenticate gate scan";
        
        // Check if biometrics (Touch ID) are available
        if ([context canEvaluatePolicy:LAPolicyDeviceOwnerAuthenticationWithBiometrics error:&error]) {
            ...
            // Wait for the block to call dispatch_semaphore_signal
