...
- debugging is cleaner: errors show up in compile time.
...
| ---- | ---- | ---- |
| Binding files | Multiple | 6 subsystems, platform-specific isolation, fast incremental builds |
| Build System | CMake | Windows SDK libraries (`winbio`, `bcrypt`), dual targets (`.pyd` + `.exe`), future cross-platform |
...
├── CMakeLists.txt                <-- CMake C++ build configuration & target definitions
└──  pyproject.toml                <-- Modern PEP 517/518 build config (enables 'pip install .')
```
...
| `test_pipeline.cpp` | **Not Bound** | Standalone C++ integration test binary. |
