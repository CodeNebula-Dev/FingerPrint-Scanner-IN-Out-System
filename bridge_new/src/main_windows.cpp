...
void do_scan() {
  ...
  // template data (only pass/fail), we simulate the scan by asking
  // which student's fingerprint to generate.
  // When real fingerprint hardware is integrated, this block will be
  ...
  if (match.match_count > 1) {
    std::cout << BOLD << YELLOW << "  [WARNING] Multiple entries detected! "
              << match.match_count << " students share this fingerprint.\n"
...
void do_view_logs() {
  ...
  for (const auto &entry : entries) {
    ...
    std::cout << std::left << std::setw(12) << entry.roll_number
              ...
              << std::setw(8) << (entry.late_return ? "YES" : "NO") << last_time
              << "\n";
  }
}
...
void do_nuke_database() {
  ...
  std::cout << BOLD << RED << "\n  !!! DANGER ZONE !!!\n"
            ...
            << "    - All fingerprint templates (.dat and .fpt files)\n"
            << "    - All daily gate logs\n"
            << "    - All home leave records\n"
            << "    - All rejection logs\n"
            << "    - Master index cache\n"
            << RESET << "\n";
  ...
  std::cout << "  Final confirmation - type 'YES' to proceed: ";
  std::string confirm2;
  std::cin >> confirm2;
